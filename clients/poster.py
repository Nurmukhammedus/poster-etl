...
Три режима получения данных:
  ...
  get_transactions()      — кассовые чеки (transactions.getTransactions)
"""
...
class PosterClient:
    ...
    def get_transactions(self, date_from: str, date_to: str) -> list[dict]:
        ...
        Структура ответа отличается от других эндпоинтов (вложенные products).
        """
        ...
        resp        = body.get("response", {})
        total_count = int(resp.get("count", 0))
        total_pages = -(-total_count // self.per_page)
